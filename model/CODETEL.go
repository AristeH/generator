...
<table id="thetable"  style="margin:20px">
  <thead><tr>
   <th .sortable>Наименование</th><th .sortable>Код</th><th .sortable>Оператор</th><th .sortable>Регион</th><th .sortable>С</th><th .sortable>По</th>
   </tr></thead>
  <tbody>`

rows, _ := config.DB.Query( `select * FROM CODETEL`)
defer rows.Close()
for rows.Next() {
   var obj CODETEL
   rows.Scan( &obj.